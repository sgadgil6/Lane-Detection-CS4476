...
def process_pipeline(frame, keep_state=True):
    ...
    _, eq_white_mask = cv2.threshold(cv2.equalizeHist(gray), thresh=178, maxval=250, type=cv2.THRESH_BINARY)
    ...
    mask = cv2.rectangle(mask, pt1=(0, 0), pt2=(thumb_w, thumb_h + 2 * off_y), color=(0, 0, 0), thickness=cv2.FILLED)
